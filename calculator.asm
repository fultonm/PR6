...
; 10.27
; This routine clears the stack by ressetting the stack pointer (R6).
...
OpMult			AND 		R3,R3,#0		; R3 holds sign of multiplier.
				...
				ADD 		R5,R5,#0		; Test for successful POP.
				BRp 		Restore4		; Failure; restore first POP.
...
OpAdd			JSR			POP			; Get first source operand.
				...
				BRp			Exit2		; Branch if not successful.
...
BinarytoASCII	LEA 		R1,ASCIIBUFF	; R1 points to string being generated.
				...
				STR 		R2,R1,#0
				BRnzp		Begin100
NegSign			LD 			R2,ASCIIminus	; First store the ASCII minus sign.
				STR 		R2,R1,#0
