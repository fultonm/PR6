...
void display_monitor_update(LC3_p lc3) {
  ...
  sprintf(cpu_strings[0].description, "x%04X", lc3->cpu->pc);
  sprintf(cpu_strings[1].label, "IR:");
  sprintf(cpu_strings[1].description, "x%04X", lc3->cpu->ir);
  sprintf(cpu_strings[2].label, "ALU A:");
  sprintf(cpu_strings[2].description, "x%04X", lc3->cpu->alu_a);
  sprintf(cpu_strings[3].label, "ALU B:");
  sprintf(cpu_strings[3].description, "x%04X", lc3->cpu->alu_b);
  sprintf(cpu_strings[4].label, "MAR:");
  sprintf(cpu_strings[4].description, "x%04X", lc3->cpu->mar);
  sprintf(cpu_strings[5].label, "MDR:");
  sprintf(cpu_strings[5].description, "x%04X", lc3->cpu->mdr);
  sprintf(cpu_strings[6].label, "CC N:");
  sprintf(cpu_strings[6].description, "%d", lc3->cpu->ccN);
...
MonitorReturn display_monitor_loop(LC3_p lc3) {
  ...
  MonitorReturn monitor_return = {MONITOR_UPDATE, breakpoints[lc3->cpu->pc]};
  ...
  while ((c = wgetch(menu_windows[active_window])) != 57) {
    ...
    case 54:
      ...
      move(MEM_PANEL_HEIGHT + HEIGHT_PADDING + 1,
           strlen(MSG_EDIT_MEM_PROMPT_DATA) + strlen(mem_addr_input) + 4);
      echo();
